...
def upgrade() -> None:
    ...
    op.create_index(
        "idx_collections_metadata", "collections", ["metadata"], postgresql_using="gin"
    )
    op.execute(
        """
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "collection_id", "video_id", name="unique_collection_video"
        ),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "collection_id", "user_id", name="unique_collection_member"
        ),
        sa.CheckConstraint("role IN ('owner', 'editor', 'viewer')", name="valid_role"),
    )

    op.create_index(
        "idx_collection_members_collection", "collection_members", ["collection_id"]
    )
