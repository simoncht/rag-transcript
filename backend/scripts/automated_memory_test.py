...
class MemoryTestRunner:
    ...
    def check_health(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"{Colors.RED}✗{Colors.END} Backend returned status {response.status_code}")
                return False
    ...
    def get_available_videos(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 401:
                print(f"{Colors.YELLOW}⚠{Colors.END}  No authentication - using Clerk bypass")
            ...
            if response.status_code == 200:
                ...
                print(f"{Colors.RED}✗{Colors.END} Failed to fetch videos: {response.status_code}")
    ...
    def create_conversation(self, video_id: str) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200 or response.status_code == 201:
                ...
                print(f"{Colors.RED}✗{Colors.END} Failed to create conversation: {response.status_code}")
                print(f"Response: {response.text[:200]}")
                return None
    ...
    def send_message(self, query: str, turn: int) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"  Turn {turn}: {Colors.GREEN}✓{Colors.END} Response received ({len(content)} chars)")
                print(f"           {display_content}")

                return True, content, metadata
            else:
                print(f"  Turn {turn}: {Colors.RED}✗{Colors.END} Failed ({response.status_code})")
                return False, "", {}
    ...
    def validate_recall(self, response: str, expected_terms: List[str], exact: bool = False) -> Tuple[bool, List[str]]:
        """
    ...
    def run_test_stage_1(self) -> Dict[str, Any]:
        ...
        if success and response_1:
            ...
            caps = [w for w in words if w and w[0].isupper() and len(w) > 2 and w not in ["The", "This", "In", "According"]]
            if caps:
    ...
    def run_test_stage_2(self, context: Dict[str, str]) -> Dict[str, Any]:
        ...
        query_10 = f"What specific points did {instructor} emphasize about this subject?"
    ...
    def run_test_stage_4(self, context: Dict[str, str]) -> Dict[str, Any]:
        ...
        passed, missing = self.validate_recall(response_40, [instructor, topic, framework])
    ...
    def generate_report(self, all_stages: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        overall_success = (overall_passed / overall_total * 100) if overall_total > 0 else 0

        # Find critical failures
        critical_failures = []
        for stage_data in all_stages:
            for result in stage_data["results"]:
                if result.get("critical_test") and not result.get("validation_passed", True):
    ...
    def print_report(self, report: Dict[str, Any]):
        ...
        for stage in report['stage_stats']:
            ...
            print(f"{stage['name']:30} {color}{stage['passed']:2}/{stage['total']:2} ({rate:5.1f}%) {bar}{Colors.END}")
        ...
        if overall >= 85 and stage_4_rate >= 80:
            print(f"{Colors.GREEN}✓ Phase 2 COMPLETE{Colors.END} - Excellent long-term memory")
        elif overall >= 70 and stage_4_rate >= 40:
            print(f"{Colors.YELLOW}✓ Phase 1 COMPLETE{Colors.END} - Good short-term, needs Phase 2 for long-term")
            print(f"  Recommendation: Implement Phase 2 (conversation facts extraction)")
        else:
            print(f"{Colors.RED}✗ Needs Improvement{Colors.END} - Memory retention below target")
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Automated 40-turn conversation memory test")
    parser.add_argument("--video-id", help="Video ID to test with (will auto-select if not provided)")
    ...
    if not video_id:
        ...
        print(f"{Colors.GREEN}✓{Colors.END} Using video: {videos[0]['title'][:50]}... (ID: {video_id})")
