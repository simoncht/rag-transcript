...
class Message(Base):
    ...
    input_tokens = Column(Integer, nullable=True)  # For assistant messages: prompt tokens
    output_tokens = Column(Integer, nullable=True)  # For assistant messages: completion tokens
    ...
    response_time_seconds = Column(Float, nullable=True)  # How long the LLM took to respond

    # RAG metadata (for assistant messages)
    chunks_retrieved_count = Column(Integer, nullable=True)  # How many chunks were retrieved
    chunks_used_count = Column(Integer, nullable=True)  # How many chunks were actually used in response

    # Additional metadata
    message_metadata = Column(JSONB, nullable=True)  # For storing extra info (e.g., streaming status, errors)
    ...
    chunk_references = relationship("MessageChunkReference", back_populates="message", cascade="all, delete-orphan")
...
class MessageChunkReference(Base):
    ...
    was_used_in_response = Column(Boolean, nullable=False, default=True)  # Whether LLM actually used this chunk
