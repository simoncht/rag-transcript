...
async def list_collections(
    ...
    for collection in collections:
        ...
        video_stats = db.query(
            ...
            Video, CollectionVideo.video_id == Video.id
        ).filter(
...
async def get_collection(
    ...
    collection_videos = db.query(CollectionVideo, Video).join(
        Video, CollectionVideo.video_id == Video.id
    ).filter(
        CollectionVideo.collection_id == collection_id,
        Video.is_deleted == False
    ).order_by(
        CollectionVideo.position.nullslast(),
        CollectionVideo.added_at.desc()
    ).all()
    ...
    for cv, video in collection_videos:
        videos.append(CollectionVideoInfo(
            id=video.id,
            title=video.title,
...
async def update_collection(
    ...
    if collection.is_default and request.name is not None:
        raise HTTPException(
            status_code=400,
            detail="Cannot rename default 'Uncategorized' collection"
        )
...
async def delete_collection(
    ...
    if collection.is_default:
        raise HTTPException(
            status_code=400,
            detail="Cannot delete default 'Uncategorized' collection"
        )

    db.delete(collection)
    db.commit()

    return {"message": "Collection deleted successfully", "collection_id": str(collection_id)}
...
async def remove_video_from_collection(
    ...
    cv = db.query(CollectionVideo).filter(
        CollectionVideo.collection_id == collection_id,
    ...
    if not cv:
        raise HTTPException(
            status_code=404,
            detail="Video not found in this collection"
        )
